...
def process_sentence(sent, parser, mode):
    ...
    for cidx in connectives:
        ...
        if len(list(nps_before_c.keys())) < 2:
            continue
        nps_after_c = {s:idx for idx, t, s in nps if idx > cidx   # after connective
                and (not has_jj(res[idx])                         # no adjective
                    or any(get_jjs(res[v]) == get_jjs(res[idx])
                        for k, v in list(nps_before_c.items())))}       # same adjectives
        ...
        if mode == 'noun':
            for np_b, xb in list(nps_before_c.items()):
                # compare to all nps before c
                for np_a, xa in list(nps_after_c.items()):
                    if np_b == np_a:
                        # find another element from /before/, which is different from 
                        xb2 = None
                        for np_b2_, xb2_ in list(nps_before_c.items()):
            ...
            if len(set(nps_before_c.keys()).intersection(list(nps_after_c.keys()))) > 0:
                # it is too easy if one of them appears again, since then the pronoun is the other one.
                reject_reasons.append("too easy")
                continue
            wsc_cands.append(formatted_oneline(res, list(nps_before_c.items())[0][1], list(nps_before_c.items())[1][1], mark_xa=False))
    return wsc_cands
